...
export const dbMethods = {
  ...
  createBooking(data) {
    ...
    const sDate = data.start_time ? new Date(data.start_time) : new Date();
    const eDate = data.end_time ? new Date(data.end_time) : new Date(Date.now() + 86400000);
    ...
    const newBooking = {
      ...
      start_time: isNaN(sDate.getTime()) ? new Date().toISOString() : sDate.toISOString(),
      end_time: isNaN(eDate.getTime()) ? new Date(Date.now() + 86400000).toISOString() : eDate.toISOString(),
      ...
      duration_hours: Number(data.duration_hours) || 24,
